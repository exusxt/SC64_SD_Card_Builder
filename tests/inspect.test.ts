...
async function makeCard(): Promise<string> {
  ...
  await writeFile(join(root, 'Tetris.gb'), Buffer.alloc(0x10))
  await writeFile(join(root, 'Kirby.gbc'), Buffer.alloc(0x10))
  await writeFile(join(root, 'Mario.smc'), Buffer.alloc(0x10))
  await writeFile(join(root, 'Sonic.sms'), Buffer.alloc(0x10))
  await writeFile(join(root, 'Sonic.gg'), Buffer.alloc(0x10))
...
describe('inspectCard', () => {
  ...
  it('inspects a card and parses the installed menu version', async () => {
    ...
    try {
      ...
      expect(info!.roms.gb).toBe(1)
      expect(info!.roms.gbc).toBe(1)
      expect(info!.roms.snes).toBe(1)
      expect(info!.roms.sms).toBe(1)
      expect(info!.roms.gg).toBe(1)
